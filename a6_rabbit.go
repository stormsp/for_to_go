...
func checkNo220(Reps *SafeMap) any {
	x:=Reps.Reps["UPS1 РЕЖ КРАС"].Value
	return(x)
}
...
func checkPrecond(Reps *SafeMap) bool {
	var x bool
	//x = 0
	if convertToInteger(Reps.Reps["РЕЖИМ ГРС КРАС"].Value) != convertToInteger(0) && convertToInteger(Reps.Reps["КРАН БАЙП КРАС"].Value) != convertToInteger(2) {
		x=x || (convertToInteger(Reps.Reps["КОНТР РЕГ КРАС"].Value) == convertToInteger(1))		// работа только на контуре САУ
	}
	return(x)
}

// mode  - 0-ручной// 1-автомат
func pid(id int, ki float32, kp float32, ref float32, pos, fdb float32, fdbtrust int, mode int) int {
	return int(pidreg(id,ki,kp,0.1,ref,pos,fdb,0,0,100,10,0,0.05,mode,4,1,0,2,0,fdbtrust))
}
...
func mainOutput(Reps *SafeMap) {
	...
	//
	mode:=checkPrecond(Reps)
	mode1 := 0
	if mode {dout[31]=1; mode1 = 1} else {dout[31]=0}
	dost1 := 0
	if Reps.Reps["РВЫХ123 КРАС"].Reliability {
		dost1 = 1
	}
	u:=pid(1,Reps.Reps["S0КИРЕГР КРАС"].Value,Reps.Reps["S1КПРЕГР КРАС"].Value,Reps.Reps["РВЫХ ЗАД КРАС"].Value,Reps.Reps["ПОЗ ЗАДВ КРАС"].Value,Reps.Reps["РВЫХ123 КРАС"].Value,dost1,mode1)
	...
	//
	if (convertToInteger(Reps.Reps["РЕЖ РЕГ04 КРАС"].Value) == convertToInteger(0)) { // тест бп при не на бп {

		u=int(Reps.Reps["РУЧПОЛ БП КРАС"].Value)	// и перемещение вслед за руч пол
		u=klap_test(u,Reps.Reps["РУЧПОЛ БП КРАС"].Value,Reps.Reps["ПОЗ ЗАДВ КРАС"].Value,33,Reps.Reps["КРАН БАЙП КРАС"].Value,6,Reps.Reps["РЕЖИМ ГРС КРАС"].Value)
		aout[32]=int(u)
		setSens(&Reps.Reps["РУЧ БП04 КРАС"].Value,float32(u),0.1)
	}
