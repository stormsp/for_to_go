...
`
	...
	code = strings.ReplaceAll(code, "x=x+Reps[\"КОМ АО КРАС\"].Value", "x=x||Reps[\"КОМ АО КРАС\"].Value")
	code = strings.ReplaceAll(code, "x=x+2*checkFire(0)", "x=x||checkFire(false)")
	code = strings.ReplaceAll(code, "x=x+3*Reps[\"РВЫХ123АВ КРАС\"].Value", "x=x||Reps[\"РВЫХ123АВ КРАС\"].Value")
	code = strings.ReplaceAll(code, "func checkFire(dummy any) any {", "func checkFire(dummy any) bool {")
	code = strings.ReplaceAll(code, "TRUE(Reps[\"ДАТА АО КРАС\"].Value)", "1")
