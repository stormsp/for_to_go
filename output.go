...
if checkPrecondSt(0) {
  ...
  dout[1]=1	// ход ао
  // пошел останов
    SET(Reps["табл АварияДЕС"].Value[sys_num], 1)
	SET(Reps["звонок шк ДЕС"].Value[sys_num], 1)
	x=setwex(Reps["ОХР КР ДЕС"].Value[sys_num],1,Reps["Т ож кран ДЕС"].Value)
    x=setwex(Reps["Вход ДЕС"].Value[sys_num],1,Reps["Т ож кран ДЕС"].Value)
    x=setwex(Reps["Выход ДЕС"].Value[sys_num],1,Reps["Т ож кран ДЕС"].Value)
    x=setwex(Reps["ВЫХ Д ДЕС"].Value[sys_num],1,Reps["Т ож кран ДЕС"].Value)
    x=setwex(Reps["КРдоРУ ДЕС"].Value[sys_num],1,Reps["Т ож кран ДЕС"].Value)
    SET(Reps["Клап котлы ДЕС"].Value[sys_num], 1)
	
 //set({}[sys_num],1 - отключение котлов
    if (Reps["Вход ДЕС"].Value == 2) && (Reps["Выход ДЕС"].Value == 2) && (Reps["ОХР КР ДЕС"].Value == 2) && (Reps["КРдоРУ ДЕС"].Value == 2) // проверка закрылись ли краны
          x=setwex(Reps["СВзаВХ ДЕС"].Value[sys_num],0,Reps["Т ож кран ДЕС"].Value)       // открыть свечной кран на входе
          x=setwex(Reps["СВдоВЫХ ДЕС"].Value[sys_num],0,Reps["Т ож кран ДЕС"].Value)      // открыть свечной кран на выходе
          x=setwex(Reps["СВ ОК ДЕС"].Value[sys_num],0,Reps["Т ож кран ДЕС"].Value)        // открыть свечной кран ОХРАН крана
          SET(Reps["ТУ КОТЕЛ 1 ДЕС"].Value[sys_num], 1)
	             // выключить котел 1
          SET(Reps["ТУ КОТЕЛ 2 ДЕС"].Value[sys_num], 1)
	             // выключить котел 2
}
dout[1]=2+((Reps["ОХР КР ДЕС"].Value == 2) && (Reps["Вход ДЕС"].Value == 2) && (Reps["КРдоРУ ДЕС"].Value == 2) && (Reps["Выход ДЕС"].Value == 2) && (Reps["ВЫХ Д ДЕС"].Value == 2) && (Reps["СВзаВХ ДЕС"].Value == 1) && (Reps["СВдоВЫХ ДЕС"].Value == 1) && (Reps["СВ ОК ДЕС"].Value == 1))  // ход ао
    SET(Reps["КОМ РЕЖ3 ДЕС"].Value[sys_num], 0)
